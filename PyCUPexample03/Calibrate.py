...
import read_res
cp.save.record_path = "RecordSWMM.rcd"
cp.save.raw_path = "RawSWMM.rst"
...
def objective_functionMP(X,read_start,read_end,read_step,n_jobs):
    ...
    inp_path = proj_dir + r"\Example3-Post.inp"
    rpt_path = proj_dir + r"\Example3-Post.rpt"
    swmm_out_path = proj_dir + r"\Example3-Post.out"
    ...
    fitness1 = cp.evaluation_metrics.OneMinusNSE(calibration_data,res_outfall)
    fitness2 = cp.evaluation_metrics.MAE(calibration_data,res_outfall)
    fitness = np.array([fitness1,fitness2]).reshape(1,-1)
...
if __name__ == "__main__":
    ...
    cp.MOPSO.EliteOppoSwitch = True
    cp.MOPSO.OppoFactor = 0.1
    cp.MOPSO.runMP(45, 11,n_obj=2, lb=lb, ub=ub, MaxIter=99, fun=objective_functionMP,
                   nAr=50,M=100, n_jobs=5, args=("2007-01-01 00:01:00", "2007-01-01 12:00:00",
                   60,  2))